...
- Gateway-client metrics now expose the `gateway_requests_in_flight` gauge. Dropped request futures only decrement the gauge because graceful shutdown and disconnected clients are not gateway failures.
- Added `--rpc.gateway-add-transaction-timeout` (default 60s) for sequencer submissions made by `starknet_addInvokeTransaction`, `starknet_addDeclareTransaction`, and `starknet_addDeployAccountTransaction`.
