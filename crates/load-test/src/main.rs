...
use pathfinder_lib::core::{
    ...
    StarknetTransactionIndex, StorageAddress,
};
use pathfinder_lib::rpc::v01::types::{
    reply::Transactions as StarknetTransactions, request::EventFilter,
};
...
async fn block_explorer(user: &mut GooseUser) -> TransactionResult {
    ...
    let block = requests::get_block_by_number(
        user,
        StarknetBlockNumber::new(block_number).expect("block_number out of range"),
    )
    .await?;
    let block_by_hash = requests::get_block_by_hash(user, block.block_hash.unwrap()).await?;
    ...
    if let StarknetTransactions::HashesOnly(hashes) = block.transactions {
        for (idx, hash) in hashes.iter().enumerate() {
            ...
            let transaction_by_hash_and_index = requests::get_transaction_by_block_hash_and_index(
                user,
            ...
            let transaction_by_number_and_index =
                requests::get_transaction_by_block_number_and_index(
                    user,
...
async fn task_block_by_number(user: &mut GooseUser) -> TransactionResult {
    requests::get_block_by_number(user, StarknetBlockNumber::new_or_panic(1000)).await?;
    Ok(())
}
...
async fn task_block_transaction_count_by_number(user: &mut GooseUser) -> TransactionResult {
    requests::get_block_transaction_count_by_number(user, StarknetBlockNumber::new_or_panic(1000))
        .await?;
    Ok(())
}
