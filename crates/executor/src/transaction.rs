...
pub(crate) fn find_l2_gas_limit_and_execute_transaction(
    ...
    let (l2_gas_limit, mut output, saved_state) =
        match simulate_transaction(tx, tx_index, tx_executor, &revert_behavior) {
            Ok((output, saved_state)) => {
                ...
                (l2_gas_adjusted, output, saved_state)
            }
    ...
    if execution_flags.charge_fee && l2_gas_limit > initial_l2_gas_limit {
        ...
        // Reset the state to what it was before the search.
        tx_executor.block_state = Some(saved_state);
        // Set the L2 gas limit to zero so that the transaction reverts with a detailed
        // `ExecutionError`.
        set_l2_gas_limit(tx, GasAmount::ZERO);
        match execute_transaction(tx, tx_index, tx_executor, ExecutionBehaviorOnRevert::Fail) {
    ...
    // on the executor.
    drop(saved_state);
    output.0.receipt.gas.l2_gas = l2_gas_limit;
