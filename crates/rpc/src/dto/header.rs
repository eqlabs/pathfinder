...
pub struct PendingBlockHeader<'a>(&'a pathfinder_common::BlockHeader);
...
impl serialize::SerializeForVersion for PendingBlockHeader<'_> {
    fn serialize(
        &self,
        serializer: serialize::Serializer,
    ) -> Result<serialize::Ok, serialize::Error> {
...
mod tests {
    ...
    fn pending_header() {
        ...
        let encoded = s.serialize(&PendingBlockHeader(&header)).unwrap();
