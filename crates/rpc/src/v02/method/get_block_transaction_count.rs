        ...
        let block_id = match input.block_id {
            BlockId::Pending => {
                let count = context
                    ...
                    .map(|x| x.body.transaction_data.len())
                    .unwrap_or_default();

                return Ok(count as BlockTransactionCount);
            }
...
mod tests {
    ...
    fn get_count(context: &RpcContext) -> BlockTransactionCount {
        context
            .pending_data
            .block_unchecked()
            .body
            .transaction_data
            .len() as BlockTransactionCount
    }
