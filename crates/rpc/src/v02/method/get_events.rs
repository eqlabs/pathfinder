...
pub async fn get_events(
    ...
    let db_events: JoinHandle<Result<_, GetEventsError>> = tokio::task::spawn_blocking(move || {
        ...
        // Check whether request's continuity token is valid. If there are no events then it was invalid,
        // and unfortunately we can only determine that after processing the query.
