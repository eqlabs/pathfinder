...
impl crate::jsonrpc::RpcMethodHandler for RpcHandlerV04 {
    async fn call_method(method: &str, ctx: RpcContext, params: serde_json::Value) -> RpcResult {
        ...
        let output = match method {
            ...
            "starknet_addDeclareTransaction"           => v04_method::add_declare_transaction.invoke(ctx, params).await,
            "starknet_addDeployAccountTransaction"     => v04_method::add_deploy_account_transaction.invoke(ctx, params).await,
            "starknet_addInvokeTransaction"            => v04_method::add_invoke_transaction.invoke(ctx, params).await,
            "starknet_getTransactionReceipt"           => v04_method::get_transaction_receipt.invoke(ctx, params).await,
            "starknet_syncing"                         => v04_method::syncing.invoke(ctx, params).await,
            "starknet_simulateTransactions"            => v04_method::simulate_transactions.invoke(ctx, params).await,
            "starknet_estimateMessageFee"              => v04_method::estimate_message_fee.invoke(ctx, params).await,
