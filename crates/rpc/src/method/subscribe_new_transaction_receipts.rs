            ...
            Ok(Some(Params {
                finality_status: value
                    .deserialize_optional_array("finality_status", |v| {
                        v.deserialize::<TxnFinalityStatus>()
                    })?
        ...
        loop {
            tokio::select! {
                    ...
                    match block {
                        ...
                        Ok(block) => {
                            ...
                            if block.block_number == last_pre_confirmed_block {
                                ...
                                for (transaction, (receipt, _events)) in block.transactions.iter().zip(block.transaction_receipts.iter()) {
                ...
                pending_changed = pending_data.changed() => {
                    ...
                    for (transaction, (receipt, _events)) in pending.transactions().iter().zip(pending.transaction_receipts_and_events().iter()) {
                        if pre_confirmed_sent_txs.contains(&transaction.hash) {
                            continue;
                        }

                        if !params.matches(&transaction.variant.sender_address(), pending.block().finality_status()) {
                            continue;
                        }
...
mod tests {
    ...
    use crate::{v09, Notifications, PendingData};
