use anyhow::Context;
use pathfinder_common::{ChainId, L1TransactionHash, TransactionHash};
use pathfinder_ethereum::{core_addr, EthereumApi};
...
pub async fn get_messages_status(context: RpcContext, input: Input) -> Result<Output, Error> {
    ...
    let core_address = match context.chain_id {
        ChainId::MAINNET => H160::from(core_addr::MAINNET),
        ChainId::SEPOLIA_TESTNET => H160::from(core_addr::SEPOLIA_TESTNET),
        _ => return Err(anyhow::anyhow!("Unsupported chain").into()),
    };

    let l1_handler_txs = ethereum
        .get_l1_handler_txs(&core_address, &input.transaction_hash)
        .await
