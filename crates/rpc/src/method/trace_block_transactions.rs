...
pub(crate) mod tests {
    ...
    pub(crate) async fn setup_multi_tx_trace_test<'a>(
    ...
    async fn test_multiple_pending_transactions() -> anyhow::Result<()> {
        ...
        let expected = TraceBlockTransactionsOutput {
            traces: traces
                .into_iter()
                .map(|t| (t.transaction_hash, t.trace_root.trace))
                .collect(),
            include_state_diffs: false,
        };
