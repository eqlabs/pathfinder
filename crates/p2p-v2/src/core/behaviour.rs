...
/// The main behaviour for the p2p network.
pub struct Behaviour<AppBehaviour: NetworkBehaviour> {
    cfg: Config,
    peers: PeerSet,
    secret: Secret,
    pending_events: VecDeque<ToSwarm<<Self as NetworkBehaviour>::ToSwarm, THandlerInEvent<Self>>>,
    /// The core behaviour.
    core: CoreBehaviour<AppBehaviour>,
}

#[derive(NetworkBehaviour)]
pub struct CoreBehaviour<AppBehaviour: NetworkBehaviour> {
    ...
    application: AppBehaviour,
}

/// The event type for the core behaviour, generated by the NetworkBehaviour derive macro.
pub type CoreEvent<B> = CoreBehaviourEvent<B>;


impl<AppBehaviour: NetworkBehaviour> Behaviour<AppBehaviour> {
    ...
    pub fn kademlia_mut(&mut self) -> &mut kad::Behaviour<MemoryStore> {
        &mut self.core.kademlia
    }

    pub fn application(&self) -> &AppBehaviour {
        &self.core.application
    }

    pub fn application_mut(&mut self) -> &mut AppBehaviour {
        &mut self.core.application
    }

    /// Get the closest peers to a given peer.
    pub fn get_closest_peers(&mut self, peer: PeerId) -> kad::QueryId {
        self.kademlia_mut().get_closest_peers(peer)
    }
...
where
    B: NetworkBehaviour,
{
    type ConnectionHandler = <CoreBehaviour<B> as NetworkBehaviour>::ConnectionHandler;
    type ToSwarm = <CoreBehaviour<B> as NetworkBehaviour>::ToSwarm;
