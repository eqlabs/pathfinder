...
#[derive(Parser, Debug)]
struct Args {
    #[arg(long, short, value_parser, env = "IDENTITY_CONFIG_FILE")]
    identity_config_file: Option<std::path::PathBuf>,
    #[arg(long, short, value_parser, env = "LISTEN_ON")]
    listen_on: Multiaddr,
    #[arg(long, short, value_parser, env = "BOOTSTRAP_INTERVAL")]
    bootstrap_interval_seconds: u64,
    #[arg(long, short, value_parser, env = "PRETTY_LOG", default_value = "false")]
    pretty_log: bool,
    #[arg(long, short, value_parser, env = "NETWORK")]
    network: Network,
}
    ...
    loop {
        ...
        tokio::select! {
            ...
            Some(event) = swarm.next() => {
                match event {
                    SwarmEvent::Behaviour(behaviour::BootstrapBehaviourEvent::Identify(e)) => {
