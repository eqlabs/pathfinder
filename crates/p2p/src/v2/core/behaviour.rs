...
use libp2p::{autonat, dcutr, identify, ping, relay, Multiaddr, PeerId};

use crate::peers::{Connectivity, Direction, KeyedNetworkGroup, Peer, PeerSet};
use crate::secret::Secret;
use crate::Config;

// TODO the outer behaviour (now: `core::Behaviour`) that does connection
// management should be extracted to a separate struct that will become the
// first sub-behaviour of what is currently called Inner. Passing events to
// other behaviours will happen via the code generated by the derive macro. Just
// like it happens with the other sub-behaviours. After that the Inner struct
// will be renamed to just `core::Behaviour`.
...
pub struct Inner<ApplicationBehaviour: NetworkBehaviour> {
    // TODO connection management behaviour goes here as the first one
    ...
    application: ApplicationBehaviour,
}

/// [`InnerEvent`] is generated by the [`NetworkBehaviour`] derive macro.
...
where
    B: NetworkBehaviour,
{
    type ConnectionHandler = <Inner<B> as NetworkBehaviour>::ConnectionHandler;
    type ToSwarm = <Inner<B> as NetworkBehaviour>::ToSwarm;
    ...
    B: NetworkBehaviour,
{
            ...
            Ok(duration) => {
                self.peers.update(event.peer, |peer| {
                    peer.min_ping = Some(match peer.min_ping {
                        Some(min_ping) => std::cmp::min(min_ping, duration),
                        None => duration,
                    });
                });
            }
    ...
    pub fn application(&self) -> &B {
        &self.inner.application
    }

    pub fn application_mut(&mut self) -> &mut B {
        &mut self.inner.application
    }
