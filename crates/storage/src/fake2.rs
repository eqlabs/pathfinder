...
/// TODO check these guarantees some of them are outdated
///
...
///     - parent hash wrt previous block, parent hash of the genesis block is
///       `0`
        ...
        if !init.is_empty() {
            for i in 1..n {
                let deployed_in_parent = init
                    ...
                    .unwrap();
                let Block { state_update, .. } = init.get_mut(i).unwrap();
