...
use crate::{fixtures, IntoFelt, IntoTypesCoreFelt as _};
...
impl Account {
        ...
        let address = if let Some(address) = address {
            ...
            ContractAddress(
                calculate_contract_address(
                    ...
                    // ),
                    Default::default(),
                    starknet_api::core::ClassHash(class_hash.0.into_types_core_felt()),
                    &starknet_api::transaction::fields::Calldata(Arc::new(vec![public_key
                        .0
                        .into_types_core_felt()])),
                    Default::default(),
                )?
                .into_felt(),
            )
    ...
    fn simulate_constructor(&self, state_update: &mut StateUpdateData) -> anyhow::Result<()> {
        let interface_storage_var = get_storage_var_address(
            ...
            &[fixtures::ISRC6_ID.into_types_core_felt()],
        );
        set_storage_at(
            state_update,
            self.address,
