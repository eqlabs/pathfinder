...
use super::{Height, MyContext, Round, ValidatorAddress, ValueId};
...
pub struct Vote {
    ...
    pub extension: Option<SignedExtension<MyContext>>,
}
...
impl malachite_types::Vote<MyContext> for Vote {
    ...
    fn extension(&self) -> Option<&SignedExtension<MyContext>> {
        self.extension.as_ref()
    }

    fn take_extension(&mut self) -> Option<SignedExtension<MyContext>> {
        self.extension.take()
    }

    fn extend(self, extension: SignedExtension<MyContext>) -> Self {
        Self {
