...
macro_rules! projective_point_str {
    ($x:expr,$y:expr) => {
        ProjectivePoint {
            x: $crate::algebra::field::montfelt_dec!($x),
            y: $crate::algebra::field::montfelt_dec!($y),
            z: $crate::algebra::field::MontFelt::ONE,
            infinity: false,
        }
    };
}
pub(crate) use projective_point_str; // export it
...
impl ProjectivePoint {
    ...
    pub fn double(&mut self) {
        ...
        let t = MontFelt::THREE * self.x * self.x + self.z * self.z;
        let u = MontFelt::TWO * self.y * self.z;
        let v = MontFelt::TWO * u * self.x * self.y;
        let w = t * t - MontFelt::TWO * v;

        let uy = u * self.y;

        let x = u * w;
        let y = t * (v - w) - MontFelt::TWO * uy * uy;
        let z = u * u * u;
