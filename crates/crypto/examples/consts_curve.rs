...
fn main() {
    ...
    println!(
        "Curve constants successfully written to {}",
        dest_path.display()
    );
    println!("Please copy this file to crypto/src/algebra/curve/consts.rs");
}
...
fn generate_consts(bits: u32) -> Result<String, std::fmt::Error> {
    ...
    write!(buf, "pub const G_CONSTS_BITS: usize = {bits};\n\n")?;

    push_points(&mut buf, bits)?;

    Ok(buf)
}

/// Pushes points to the buffer for a given base point.
fn push_points(buf: &mut String, bits: u32) -> std::fmt::Result {
    ...
    writeln!(
        buf,
        "#[rustfmt::skip]\npub const G_CONSTS: [AffinePoint; {len}] = ["
    )?;
