...
pub async fn sync_ethereum_state(
    ...
    sync_ethereum_state_impl(eth_api, tx_event).await
}
...
mod tests {
    ...
    mod sync_ethereum_state_impl {
        ...
        async fn happy_path() {
            ...
            tokio::spawn(sync_ethereum_state_impl(mock_fetcher, tx_event));

            match rx_event.recv().await.unwrap() {
                Event::Updates(recv) => assert_eq!(recv, logs1),
                _other => panic!("Expected Updates event"),
            }

            match rx_event.recv().await.unwrap() {
                Event::Updates(recv) => assert_eq!(recv, logs2),
                _other => panic!("Expected Updates event"),
            }
        }
        ...
        async fn shutdown() {
            ...
            let handle = tokio::spawn(sync_ethereum_state_impl(mock_fetcher, tx_event));
        ...
        mod reorg {
            ...
            async fn partial() {
                ...
                match rx_event.recv().await.unwrap() {
                    Event::Reorg(recv_tail) => assert_eq!(recv_tail, expected_tail.block_number),
