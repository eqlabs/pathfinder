...
pub async fn download_class<SequencerClient: GatewayApi>(
    ...
    match (hash, serialized_class) {
        ...
        (ComputedClassHash::Sierra(hash), SerializedClassDefinition::Sierra(sierra_definition)) => {
            ...
            // FIXME(integration reset): work-around for integration containing
            // Sierra classes that are incompatible with production
            // compiler. This will get "fixed" in the future by
            // resetting integration to remove these classes at
            // which point we can revert this.
            //
            // The work-around ignores compilation errors on integration, and
            // instead replaces the casm definition with empty
            // bytes.
