use crate::state::block_hash::{verify_block_hash, BlockExtra};
...
pub enum Event {
    ...
    Update((Box<Block>, BlockExtra), Box<StateUpdate>, Timings),
        ...
        Ok(MaybePendingBlock::Block(block)) => {
            ...
            match block.status {
                Status::AcceptedOnL1 | Status::AcceptedOnL2 => Ok(DownloadBlock::Block(
                    block,
                    verify_result.get_extra().unwrap_or_default(),
                )),
                _ => Err(anyhow!(
