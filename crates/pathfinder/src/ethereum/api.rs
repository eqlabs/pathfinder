...
pub trait Web3EthApi {
    ...
    async fn logs(&self, filter: Filter) -> std::result::Result<Vec<Log>, GetLogsError>;
        ...
        retry(
            || {
                self.0.eth().logs(filter.clone()).map_err(|e| match e {
                    ...
                    _ => GetLogsError::Other(e),
                })
            },
            |e| match e {
                GetLogsError::Other(Error::Decoder(msg)) if msg == DECODER_ERR => {
                    tracing::trace!("Spurious L1 log decoder error occurred, retrying");
                    true
                }
                GetLogsError::Other(error) => log_and_always_retry(error),
                _ => false,
            },
        )
        .await
