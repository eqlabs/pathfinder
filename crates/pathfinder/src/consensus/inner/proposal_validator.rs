...
mod tests {
    ...
    //! duplicates, empty batches. Execution, transaction mapping, and DB
    //! persistence are covered by the `p2p_task_tests` integration tests.
    //!
    //! How it works:
    ...
    proptest! {
        ...
        // An empty proposal [Init, Fin] with random proposer/valid_round
        // must be fully accepted: Init returns Accepted, Fin returns
        // EmptyProposal, and the proposer address is captured.
        #[test]
        fn valid_empty_proposals_are_accepted(seed in any::<u64>()) {
            let parts = create_valid_empty_proposal(seed);
            let mut validator = ProposalPartsValidator::new(HeightAndRound::new(0, 0));
            ...
            for (i, part) in parts.iter().enumerate() {
                ...
                if i < last_idx {
                    ...
                    prop_assert_eq!(result, ValidationResult::EmptyProposal,
                        "Last part should be EmptyProposal");
                }
            }

            // Verify data accessors and parts are stored
            prop_assert!(validator.proposer_address().is_some());
            prop_assert_eq!(validator.valid_round(), extract_valid_round(&parts));
            prop_assert_eq!(validator.parts().len(), 2);
        }
