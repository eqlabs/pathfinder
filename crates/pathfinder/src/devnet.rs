...
/// Initializes a devnet DB. The following contracts are predeclared and
/// predeployed: Cairo 1 account, ETH and STRK ERC20s, and the UDC. The
/// following contract is already declared but not deployed: Hello Starknet.
...
fn predeploy_contracts(
    ...
    fixtures::PREDECLARED_CLASSES
        .iter()
        .copied()
        .try_for_each(|(class, class_hash)| {
            class::predeclare(db_txn, state_update, class, Some(class_hash))
        })?;
    fixtures::PREDEPLOYED_CONTRACTS
        .iter()
        .copied()
        .try_for_each(|(contract_address, class_hash)| {
            contract::predeploy(state_update, contract_address, class_hash)
        })?;
    fixtures::ERC20S
        .iter()
        .copied()
    ...
    .iter()
    .copied()
    .try_for_each(|(private_key, class_hash, address)| {
...
pub mod tests {
    ...
    use crate::devnet::account::Account;
    use crate::devnet::class::preprocess_sierra;
    use crate::devnet::{class, contract, fixtures};
