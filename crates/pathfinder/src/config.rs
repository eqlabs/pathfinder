...
struct ConsensusCli {
    ...
    #[arg(
        long = "consensus.generate-dummy-proposals",
        value_name = "Percentage",
        long_help = "The number of dummy proposals to generate on startup if the node is a proposer.",
        action = clap::ArgAction::Set,
        default_value = "10",
        value_parser = clap::value_parser!(u32).range(0..=1000),
        env = "PATHFINDER_CONSENSUS_GENERATE_DUMMY_PROPOSALS",
    )]
    num_generated_proposals: u32,
}
...
pub struct ConsensusConfig {
    ...
    /// Number of dummy proposals generated by the node on startup if the node
    /// is a proposer
    pub num_generated_proposals: u32,
}
    ...
    fn parse_or_exit(consensus_cli: ConsensusCli) -> Option<Self> {
        consensus_cli.is_consensus_enabled.then(|| {
            ...
            Self {
                ...
                num_generated_proposals: consensus_cli.num_generated_proposals,
            }
        })
    }
}
